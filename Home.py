...
st_autorefresh(
    interval=refresh_interval, limit=None, key="auto-refresh-handler", debounce=True
)
...
def drawLogin():
    session_details = st.session_state.localStorage.getItem("sDetails")
    if session_details is not None:
        LoggedIn = session_details["loggedIn"]
        print(session_details)
        if(LoggedIn == True):
            lastTime = session_details["time"]
            print("CHek-2")
            currentTime = time.time()
            if (currentTime - lastTime) < 3600:
                details = {"loggedIn": True, "time": time.time()}
                check= st.session_state.localStorage.setItem("sDetails", details)
